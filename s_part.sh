...
while pacman -S archlinux-keyring linux-okhsunrog linux-okhsunrog-headers zfs-utils-okhsunrog --noconfirm; [[ $? -ne 0 ]];
do
	echo "Trying again"
  sleep 5
done
...
#chaotic-aur
while pacman -S anki nerd-fonts-jetbrains-mono swaylock-git librewolf ungoogled-chromium chromium-widevine slack-electron virtualbox-ext-oracle clipman fcft foot i3status-rust libdispatch gimp-git numix-icon-theme-git qt5-styleplugins systray-x-git yay zoom --noconfirm --needed; [[ $? -ne 0 ]];
do
	echo "Trying again"
  sleep 5
done
